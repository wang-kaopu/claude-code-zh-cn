# Native latest candidate failure: 2.1.165

- Run: https://github.com/wang-kaopu/claude-code-zh-cn/actions/runs/27001316564
...
```markdown
# Upstream text diff: 2.1.156 -> 2.1.165

- Added upstream strings: 10377
- Removed upstream strings: 14812
- Already covered by translations: 89
- Needs translation review: 10288
- Sensitive review hints: 186
...
- --client-id <id>
- --client-secret
- --config ${...}: "${...}" is not a boolean (use true/false, 1/0, yes/no, on/off)
- --config expects KEY=VALUE, got "${...}". Use --config key=value (repeatable).
- --config validation failed: ${...}
- --config was given but plugin "${...}" failed to load after install \u2014 run `claude plugin list` to see why.
...
```bash
node scripts/verify-upstream-compat.js --baseline 2.1.165 --skip-latest --native-macos-arm64 --json
node scripts/generate-upstream-text-diff.js --to 2.1.165 --native-macos-arm64
